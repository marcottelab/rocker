...
public:
    ...
    map<uint,auc_info> process_results(float threshold) {
        ...
        if (divide_roc_area_by > 0) update.mean_roc_area = temp_auc_accum / (double)(divide_roc_area_by);
        else                        update.mean_roc_area = 0;
        
        if (divide_pr_area_by > 0)  update.mean_pr_area  = temp_pr_accum / (double)(divide_pr_area_by);
        else                        update.mean_pr_area  = 0;
